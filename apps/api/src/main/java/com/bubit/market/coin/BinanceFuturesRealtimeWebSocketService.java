...
public class BinanceFuturesRealtimeWebSocketService implements WebSocket.Listener {
    ...
    private synchronized void connect() {
        ...
        connecting = true;
        try {
            WebSocket opened = WsConnect.open(httpClient, BINANCE_FUTURES_WS, this);
    ...
    private void subscribeDailyKlines(WebSocket webSocket) {
        try {
            HttpRequest request = HttpRequest.newBuilder(BINANCE_FUTURES_TICKERS).timeout(Duration.ofSeconds(10)).GET().build();
            ...
            log.info("Binance Futures 구독 복원 완료({}종목)", params.size());
        } catch (Exception e) {
            // 구독 전송 실패는 연결 실패로 취급 — 소켓을 끊어 재연결·전체 재구독. 예전엔 뒤쪽 chunk 실패 시 나머지 구독을 포기한 채 연결이 성공으로 남았다
            log.warn("Binance Futures 일봉 구독 전송 실패 — 연결 실패로 취급, 재연결: {}", e.getMessage());
            webSocket.abort();
        }
    }
    ...
    private void handleMessage(String message) {
        try {
            ...
            lastTickerReceivedAt = System.currentTimeMillis();
            payload.put("receivedAt", System.currentTimeMillis()); // 스냅샷 나이 판정용 — REST 병합은 60초 안 것만
    ...
    private void scheduleReconnect() {
        ...
        if (delay < 0) return;
        log.info("Binance Futures WS 재연결 예약({}회째, {}ms 후)", reconnect.attempts(), delay);
    ...
    public CompletionStage<?> onClose(WebSocket webSocket, int statusCode, String reason) {
        // 종료 중이거나 이미 교체된 옛 소켓의 종료면 재연결하지 않는다(리스너 소유권,)
