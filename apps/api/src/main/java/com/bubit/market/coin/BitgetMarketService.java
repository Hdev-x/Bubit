...
public class BitgetMarketService extends AbstractMarketService {
    ...
    private final Map<String, CompletableFuture<Object>> deepCandleInflight = new ConcurrentHashMap<>();
    private static final long DEEP_CANDLE_WAIT_SECONDS = 15;

    @Autowired
    public BitgetMarketService() {
        this(WebClient.builder()
                .baseUrl("https://api.bitget.com")
                .codecs(BitgetMarketService::configureLargeResponseBuffer)
                .build());
    }

    BitgetMarketService(WebClient bitgetClient) {
    ...
    public Object getCandles(String symbol, String granularity, String limit, String endTime, String productType) {
        ...
        if (!historical && isLargeGranularity(resolvedGranularity)) {
            ...
            CompletableFuture<Object> existing = deepCandleInflight.putIfAbsent(cacheKey, mine);
            if (existing != null) {
                try { return existing.get(DEEP_CANDLE_WAIT_SECONDS, TimeUnit.SECONDS); }
            ...
            try {
                ...
                Object out = loadDeepCandles(symbol, resolvedGranularity, isFutures, productType, cacheKey);
                mine.complete(out);
                return out;
                ...
                deepCandleInflight.remove(cacheKey, mine);
            }
        }
    ...
    private Object loadDeepCandles(String symbol, String resolvedGranularity, boolean isFutures, String productType, String cacheKey) {
