 ...
 * Spring singleton으로 두어 티커·호가·캔들·WS 부트스트랩이 같은 차단 상태를 본다.
 *
 * 문제: 브라우저가 0.5초마다 호가를 요청하고 서버는 요청마다 Binance를 다시 불러
 ...
 * - 같은 key의 동시 요청은 한 번만 상류로 나가고 결과·예외를 공유한다(in-flight future,).
 * - 429/418을 받으면 Retry-After(초)만큼(없으면 429=30초, 418=300초) 상류 요청을 멈추고,
 * 캐시가 staleMaxMs 안이면 오래된 값이라도 돌려준다. 없으면 emptyValue. 차단 시각 갱신은 원자적.
 * - in-flight 소유자가 된 뒤 캐시·차단을 다시 확인하고, 상류가 null이면 기존 캐시를 지우지 않는다.
 * - 캐시는 MAX_ENTRIES를 넘으면 오래된 항목부터 지운다.
*/
@Slf4j
@Component
public class BinanceRestGuard {
    ...
    /** get 결과 + stale 여부. stale = 캐시 TTL 안의 신선한 값이나 상류 성공값이 아니라 차단·실패·빈 응답 때문에 돌려준 마지막 값(또는 emptyValue). */
    public record Result(Object data, boolean stale) {}
    ...
    public BinanceRestGuard() { this(System::currentTimeMillis); }

    /** 테스트용 — 시계를 주입해 stale 경계를 sleep 없이 고정한다.*/
    BinanceRestGuard(LongSupplier clock) { this.clock = clock; }
    ...
    public Object get(String key, long ttlMs, long staleMaxMs, Supplier<Object> upstream, Object emptyValue) throws Exception {
        return getWithMeta(key, ttlMs, staleMaxMs, upstream, emptyValue).data();
    }

    /** get과 같되 stale 여부를 함께 돌려준다 — 호출자가 stale 값을 '새 성공'으로 다시 캐시하지 않게(price-precision).*/
    public Result getWithMeta(String key, long ttlMs, long staleMaxMs, Supplier<Object> upstream, Object emptyValue) throws Exception {
        long now = now();
        Entry hit = cache.get(key);
        if (isFresh(hit, now, ttlMs)) return new Result(hit.data, false);
        if (now < blockedUntil.get()) return new Result(staleOr(hit, now, staleMaxMs, emptyValue), true);

        CompletableFuture<Result> mine = new CompletableFuture<>();
        CompletableFuture<Result> existing = inflight.putIfAbsent(key, mine);
        if (existing != null) {
            ...
            try {
                ...
                return new Result(staleOr(key, staleMaxMs, emptyValue), true); // 대기한 만큼 나이가 늘었으니 지금 시각으로 다시 판정
            }
        }
        try {
            // 소유자가 된 뒤 다시 확인 — 첫 검사와 putIfAbsent 사이에 다른 소유자가 캐시를 채웠거나 차단이 시작됐을 수 있다.
            beforeOwnerRecheck();
            now = now();
            hit = cache.get(key);
            if (isFresh(hit, now, ttlMs)) return complete(mine, new Result(hit.data, false));
            if (now < blockedUntil.get()) return complete(mine, new Result(staleOr(key, staleMaxMs, emptyValue), true));

            Object data = upstream.get();
            if (data != null) {
                put(key, data);
                return complete(mine, new Result(data, false));
            }
            // 상류가 null(빈 응답)이면 있던 캐시를 지우지 않고 그대로 준다. 나이는 응답 시점 기준.
            return complete(mine, new Result(staleOr(key, staleMaxMs, emptyValue), true));
    ...
    private static Result complete(CompletableFuture<Result> f, Result out) {
        f.complete(out);
        return out;
    }
    ...
    public Object staleOr(String key, long maxAgeMs, Object emptyValue) {
        return staleOr(cache.get(key), now(), maxAgeMs, emptyValue);
    }
