...
import type { IChartApi, ISeriesApi, ISeriesMarkersPluginApi, Logical, Time, UTCTimestamp, LineData } from 'lightweight-charts';
...
import { getDerivedThemeColors } from './settings/theme';
...
import { hexToRgba } from './indicators/settings';
...
import { loadStoredDrawings, saveStoredDrawings } from './drawingStorage';
...
type Props = {
  ...
  legacyDrawingStorageKey?: string;
...
const MarketChart = forwardRef<MarketChartRef, Props>(function MarketChart({
  ...
  const resetDrawingHistory = useCallback((manager: DrawingManager) => {
    currentStateRef.current = JSON.stringify(manager.exportDrawings());
  ...
  useImperativeHandle(ref, () => ({
    ...
    selectDrawing: (id) => {
      ...
      if (manager) {
        if (id) {
          manager.deselectAll();
          const drawing = manager.getAllDrawings().find((d) => d.id === id);
    ...
    undo: () => {
      ...
      manager.importDrawings(JSON.parse(prevStateStr), (type: string, d: SerializedDrawing) => {
    ...
    redo: () => {
      ...
      manager.clearAll();
      const registry = getToolRegistry();
      manager.importDrawings(JSON.parse(nextStateStr), (type: string, d: SerializedDrawing) => {
        const entry = registry.get(type);
        return entry ? entry.factory(d.id, d.anchors, d.style, d.options) : null;
      });
  ...
  useEffect(() => {
    ...
    if (drawingStorageKey) {
      try {
        const saved = loadStoredDrawings(localStorage, drawingStorageKey, legacyDrawingStorageKey);
        if (saved) {
    ...
    resetDrawingHistory(manager);
  }, [drawingStorageKey, legacyDrawingStorageKey, resetDrawingHistory]);
  ...
  useLayoutEffect(() => {
    ...
    if (interactionBlocked) {
      ...
      snapDotRef.current?.set(null);
    }
    drawingManagerRef.current?.setHitTestEnabled(!interactionBlocked && !activeToolRef.current);
  }, [interactionBlocked]);
  ...
  useEffect(() => {
    ...
    if (drawingManagerRef.current?.isAttached()) {
      ...
      drawingManagerRef.current.setHitTestEnabled(!interactionBlockedRef.current && !activeTool);
    }
  }, [activeTool]);
  ...
  // ── 드로잉 자석 + 키보드 — useDrawingMagnet ──
  const { magnetRef, snapPriceRef } = useDrawingMagnet({ magnet, interactionBlockedRef, seriesRef, candlesRef, drawingManagerRef, previewDrawingRef, pendingAnchorsRef, selectedDrawingIdRef, setSelectedDrawingId });

  // ── RSI 캔들(하단 페인1) — useRsiPane ──
  ...
  useEffect(() => {
    ...
    timeScale.coordinateToTime = (x: number) => {
      ...
      if (!time && candlesRef.current && candlesRef.current.length > 0) {
        ...
        if (logical !== null) {
          ...
          const lastX = timeScale.timeToCoordinate(toChartTime(lastCandle.time));
          ...
          time = toChartTime(lastUnix + (diffBars * (currentTfSeconds || 86400)));
        }
      }
      return time;
    };
    ...
    timeScale.timeToCoordinate = (time: Time) => {
      ...
      if (x === null && candlesRef.current && candlesRef.current.length > 0) {
        ...
        const lastUnix = typeof lastCandle.time === 'number' ? lastCandle.time : Math.floor(new Date(lastCandle.time).getTime() / 1000);
        const offsetSeconds = -new Date().getTimezoneOffset() * 60;
        // 문자열·BusinessDay 시각은 확장 대상이 아니다(원래 코드도 timestamp 없는 값은 건너뜀)
        const targetUnix = typeof time === 'number' ? time : (typeof time === 'object' && 'timestamp' in time ? Number(time.timestamp) : 0);
        if (targetUnix) {
          const diffSeconds = targetUnix - (lastUnix + offsetSeconds);
          if (diffSeconds > 0) {
            const diffBars = diffSeconds / (currentTfSeconds || 86400);
            const lastX = originalTimeToCoordinate(toChartTime(lastCandle.time));
            const lastLogical = lastX !== null ? timeScale.coordinateToLogical(lastX) : candlesRef.current.length - 1;
            if (lastLogical !== null) {
              x = timeScale.logicalToCoordinate((lastLogical + diffBars) as Logical); // Logical은 branded number — 라이브러리 명목 타입
            }
          }
        }
      }
    ...
    manager.setHitTestEnabled(!interactionBlockedRef.current && !activeToolRef.current);
    ...
    if (storageKey) {
      try {
        const saved = loadStoredDrawings(localStorage, storageKey, legacyDrawingStorageKey);
        if (saved) {
          const data = JSON.parse(saved) as SerializedDrawing[];
          const registry = getToolRegistry();
          manager.importDrawings(data, (type, d) => {
    ...
    const saveDrawings = () => {
      ...
      if (!key) return;
      saveStoredDrawings(localStorage, key, JSON.stringify(manager.exportDrawings()));
    };
    ...
    const unsubs = [
      ...
      manager.on('drawing:cleared', () => {
        const key = drawingStorageKeyRef.current;
        if (key) saveStoredDrawings(localStorage, key, '[]');
        takeSnapshot();
      }),
    ...
    chart.subscribeCrosshairMove((param) => {
      ...
      if (data) {
        ...
        onCrosshairMoveRef.current({
          ...
          // 캔들 시리즈면 OHLC, 라인 시리즈면 value. 활성 시리즈 데이터엔 둘 중 하나가 항상 있다(whitespace는 미래 시간축 전용 시리즈)
          open: 'open' in data ? data.open : ('value' in data ? data.value : NaN),
          high: 'high' in data ? data.high : ('value' in data ? data.value : NaN),
          low: 'low' in data ? data.low : ('value' in data ? data.value : NaN),
          close: 'close' in data ? data.close : ('value' in data ? data.value : NaN),
          volume: volData && 'value' in volData ? Number(volData.value) || 0 : 0
        });
      }
    });
    ...
    const host = hostRef.current; // cleanup에서 같은 노드를 쓰도록 effect 시점에 고정 (exhaustive-deps ref 경고)
    if (host) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- 차트 생성은 variant·chartType·locked에서만. 나머지(설정·ref·훅 함수)는 생성 시점 값을 쓰고 각자 effect가 갱신한다
  ...
  useEffect(() => {
    ...
    const blockScroll = interactionBlocked || !!activeTool || !!selectedDrawingId;
    host.style.cursor = activeTool ? 'crosshair' : '';
    chart.applyOptions({
      handleScroll: blockScroll ? false : !locked,
      handleScale: blockScroll ? false : !locked,
    });
  }, [activeTool, selectedDrawingId, locked, interactionBlocked]);
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- 데이터 effect는 candles·chartType에서만 setData. 설정(bb·ma·period 등)은 useIndicators 등 각자 effect가 반영한다
  }, [candles, chartType]);
  ...
  // 차트 초기화·거래량 effect 뒤에 등록해야 첫 렌더에서도 그려진다(수정 — 훅으로 옮기며 순서가 앞당겨졌던 것을 원위치).
  useEffect(() => {
