...
// useCoinCandles의 키 가드·병합 규칙을 순수 함수로 분리 . 훅은 이 함수들을 부르기만 한다.
 ...
 * 봉을 건너뛰었거나 간격이 어긋나면 직접 붙이지 않고 REST 재조회로 메꾼다(: Bitget 주봉에서 WS가 만든 봉이
 ...
 * time 기준 병합. REST 대기 중 갱신된 OHLC/거래량만 보존하고 나머지는 REST로 보정한다.
 * TF 간격에 어긋난 봉(다른 TF 틱이 만든 가짜 봉)과 2봉 초과 미래 봉은 제외.
 * (1M은 간격이 가변이라 제외. 상대 간격 기준이라 빗썸 KST 시프트·3D 앵커에도 안전)
 */
export function mergeRefresh(
  prev: Candle[], next: Candle[], tf: string,
  updates?: { prices: ReadonlySet<number>; volumes: ReadonlySet<number> },
): Candle[] {
  ...
  for (const c of next) {
    ...
    byTime.set(time, current && updates ? {
      ...c,
      ...(updates.prices.has(time) ? { open: current.open, high: Math.max(c.high, current.high), low: Math.min(c.low, current.low), close: current.close } : {}),
      volume: updates.volumes.has(time) ? current.volume : c.volume,
    } : c);
  }
