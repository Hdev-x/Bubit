...
// MarketChart 전용 — 드로잉 자석(약자석) 스냅 + 키보드(Delete/Escape). MarketChart.tsx에서 옮김 .
// ref들은 MarketChart가 소유하고(차트 초기화 effect가 같이 씀) 여기선 읽기·갱신만 한다.
export function useDrawingMagnet({ magnet, interactionBlockedRef, seriesRef, candlesRef, drawingManagerRef, previewDrawingRef, pendingAnchorsRef, selectedDrawingIdRef, setSelectedDrawingId }: {
  magnet: boolean;
  ...
  }, [drawingManagerRef, interactionBlockedRef, pendingAnchorsRef, previewDrawingRef, selectedDrawingIdRef, setSelectedDrawingId]);
