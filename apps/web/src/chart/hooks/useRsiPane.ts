...
// MarketChart 전용 — RSI 캔들(하단 페인1) 시리즈 생성·제거·데이터·기준선. MarketChart.tsx에서 옮김 .
...
export function useRsiPane({ chartRef, candlesRef, rsiSettings, showRsiCandles, toChartTime, rsiSeriesRef, rsiPriceLinesRef, rsiSettingsRef, showRsiCandlesRef, rsiLastCountRef, rsiLastTimeRef }: {
  ...
  useEffect(() => { rsiSettingsRef.current = rsiSettings; }, [rsiSettings, rsiSettingsRef]);
  ...
  }, [rsiPriceLinesRef, rsiSettingsRef]);

  const rsiCandleColorOpts = useCallback(() => {
  ...
  const ensureRsiSeries = useCallback((): ISeriesApi<'Candlestick'> | null => {
    ...
    return s;
  }, [applyRsiLines, chartRef, rsiCandleColorOpts, rsiSeriesRef, rsiSettingsRef]);
  ...
  }, [chartRef, rsiLastCountRef, rsiLastTimeRef, rsiPriceLinesRef, rsiSeriesRef]);
  ...
  }, [rsiLastCountRef, rsiLastTimeRef, rsiSeriesRef, rsiSettingsRef, toChartTime]);
  ...
  }, [showRsiCandles, ensureRsiSeries, destroyRsiSeries, drawRsi, candlesRef, chartRef, showRsiCandlesRef]);
  ...
  useEffect(() => {
    ...
    rsiLastTimeRef.current = null;
    drawRsi(candlesRef.current);
  }, [rsiSettings, applyRsiLines, drawRsi, candlesRef, rsiCandleColorOpts, rsiLastCountRef, rsiLastTimeRef, rsiSeriesRef]);
