...
// 하모닉 패턴 렌더 헬퍼 — 색·키·라벨·TP/SL 선·완성/진행 도형 조립. useAutoPatterns.ts에서 분리 .
...
export function buildCompletedEmergingShapes(
  ...
  // SL이탈 완성도 원색으로 — 사용자 요청. 종료 구분은 라벨(SL 이탈/Hunted)·SL선 빨강으로 충분.
...
export function buildTrackerFocusShapes(
  ...
  const dTime = x.D?.time ?? tracker.przHitTime ?? tracker.entryTime ?? tracker.exitTime ?? x.C.time;
  ...
  const snapIdx = (rawT: number): number => {
    let best = 0, bestD = Infinity;
    candles.forEach((c, i) => { const d = Math.abs(Number(c.time) - rawT); if (d < bestD) { bestD = d; best = i; } });
    return best;
  };
  const snapRaw = (rawT: number): number => Number(candles[snapIdx(rawT)].time);
  // PivotPoint 형태(type·i·time·price) — 강세 패턴은 X 저점·A 고점·B 저점·C 고점, 약세는 반대. i는 스냅한 캔들 index (: any 제거)
  const isBull = tracker.type === 'bull';
  const pivot = (pt: { time: number; price: number }, low: boolean) => {
    const i = snapIdx(pt.time);
    return { type: (low ? 'low' : 'high') as 'low' | 'high', i, time: toChartTime(Number(candles[i].time)), price: pt.price };
  };
  // buildCompletedEmergingShapes가 먹는 형태로 변환. X/A/B/C는 chart-time으로, D는 raw(함수가 변환).
  const pseudo: Parameters<typeof buildCompletedEmergingShapes>[0] = {
    points: { X: pivot(x.X, isBull), A: pivot(x.A, !isBull), B: pivot(x.B, isBull), C: pivot(x.C, !isBull) },
    isBullish: isBull,
    error: 0, przMin: dPrice, przMax: dPrice, // 도형 그리기엔 쓰이지 않는 필수 필드 — 타입 충족용
    ...
    // 도형은 'timeout'만 라벨로 쓴다 — 그 외 문자열(cancelled·invalidated 등)은 원래도 무시됐다
    endReason: tracker.exitReason === 'sl' || tracker.exitReason === 'tp' || tracker.exitReason === 'timeout' ? tracker.exitReason : undefined,
    slBroken: tracker.slBroken === true,
    slHunted: tracker.slHunted === true,
    abcdRatio: tracker.abcdRatio,
  };
