...
export function useIndicators({
  ...
  useEffect(() => {
    ...
    // candles는 deps에서 제외 — 매 틱 전체 setData 방지(실시간 갱신은 MarketChart 담당). 설정(maSettings=종류/기간/색) 변경 시에만 재계산.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- candles 제외(위 설명)
  }, [maSettings, locked, drawingStorageKey, variant, chartType, chartRef, maSeriesMapRef, tickDecimalsRef, toChartTime]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- candles·period 제외 — MA와 같은 이유(매 틱 재계산 방지, 실시간 갱신은 MarketChart)
  }, [bbSetting, locked, drawingStorageKey, variant, chartType, bbSeriesRef, chartRef, toChartTime]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- TF 스케일은 버튼 선택 직후가 아니라 새 캔들 시각이 도착할 때 적용한다(위 설명).
