...
// MarketChart 전용 — 현재가 태그·카운트다운 "값" 갱신(250ms). MarketChart.tsx에서 옮김 .
...
export function useValueOverlay({ active, currentTfSeconds, chartTheme, chartType, candles, candlesRef, tickDecimalsRef, priceTagRef, priceTagStateRef, countdownRef }: {
  ...
  }, [active, currentTfSeconds, chartTheme, chartType, candles.length, candlesRef, countdownRef, priceTagRef, priceTagStateRef, tickDecimalsRef]);
}
