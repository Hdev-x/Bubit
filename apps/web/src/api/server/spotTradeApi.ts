...
// (UI 작업용 더미 현물 보유 데이터는 사용자 요청으로 제거 — 실데이터만 표시)
...
/** /api/user/spot-trade 응답 — 필드가 빠지거나 타입이 어긋날 수 있어 전부 optional*/
type SpotTradeResponse = { hasKey?: unknown; holdings?: SpotHolding[]; orders?: SpotOrder[]; usdtAvailable?: unknown };
...
export async function fetchSpotTrade(exchange = 'BITGET'): Promise<SpotTradeOverview> {
  try {
    const [data, manual] = await Promise.all([
      authedGetJson<SpotTradeResponse | null>(`/api/user/spot-trade?exchange=${exchange}`),
