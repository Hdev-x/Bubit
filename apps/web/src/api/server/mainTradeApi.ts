...
/** /api/user/main-trade 응답 — 필드가 빠지거나 타입이 어긋날 수 있어 전부 optional*/
type MainTradeResponse = { hasKey?: unknown; positions?: MainPosition[]; orders?: MainOrder[]; planOrders?: MainPlanOrder[]; available?: unknown; equity?: unknown };
...
export async function fetchMainTrade(exchange = 'BITGET'): Promise<MainTradeOverview> {
  try {
    const data = await authedGetJson<MainTradeResponse | null>(`/api/user/main-trade?exchange=${exchange}`);
    return {
      hasKey: !!data?.hasKey,
