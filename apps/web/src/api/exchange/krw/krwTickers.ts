...
async function backendKrwTickers(ex: 'upbit' | 'bithumb', signal?: AbortSignal): Promise<KrwRow[]> {
  const token = getToken();
  const timeoutSignal = AbortSignal.timeout(10_000);
  const res = await fetch(`/coin/api/${ex}/tickers`, {
...
export async function fetchUpbitSpotTickers(signal?: AbortSignal): Promise<CoinTicker[]> {
  // 백엔드 캐시 집약(서버가 업비트 1콜로 받아 뿌림). 실패 시 throw → 호출부가 이전 목록 유지.
  const rows = await backendKrwTickers('upbit', signal);
...
// ───────────────── 빗썸 ─────────────────
export async function fetchBithumbSpotTickers(signal?: AbortSignal): Promise<CoinTicker[]> {
  // 백엔드 캐시 집약(서버가 빗썸 1콜로 받아 뿌림 + 업비트 한글명 보강). 실패 시 throw → 이전 목록 유지.
  const rows = await backendKrwTickers('bithumb', signal);
  return rows.map(krwRowToTicker)
    .filter((t) => t.last > 0)
    .sort((a, b) => (b.volume || 0) - (a.volume || 0));
}
...
export async function fetchUpbitCandles(symbol: string, granularity: string, limit: number, endTime?: string): Promise<Candle[]> {
  const token = getToken();
  const params = new URLSearchParams({ symbol, granularity, count: String(Math.min(limit, 1200)) });
