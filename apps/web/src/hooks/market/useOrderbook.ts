...
// 그대로 두면 차단 중 몇 분 전 호가가 실시간처럼 남는다. 0.5초 폴링 기준 1.5초.
...
export function useOrderbook(
  ...
  const [bookState, setBookState] = useState<{ requestKey: string; book: OrderbookSnapshot | null }>(() => ({ requestKey: bookKey, book: null }));
  let book = bookState.book;
  if (symbol && enabled && bookState.requestKey !== bookKey) {
    book = clearOnChange ? null : book;
    setBookState({ requestKey: bookKey, book });
  }
  ...
  useEffect(() => {
    ...
    if (exchange === 'UPBIT' || exchange === 'BITHUMB') {
      const sub = subscribeKrwOrderbook(exchange, symbol, (snap) => {
        if (!cancelled) setBookState({ requestKey: bookKey, book: { ...snap, key: bookKey } });
      });
    ...
    const poll = async () => {
      ...
      if (snap) {
        emptyPolls = 0;
        setBookState({ requestKey: bookKey, book: { ...snap, key: bookKey } }); // key 태그 — 호출부가 "현재 종목 호가인지" 판별
      } else if (++emptyPolls === EMPTY_POLLS_TO_CLEAR) {
        setBookState({ requestKey: bookKey, book: null }); // 오래된 호가를 실시간처럼 보이게 두지 않는다
      }
  ...
  }, [symbol, precision, isFutures, enabled, exchange, clearOnChange, bookKey]);
