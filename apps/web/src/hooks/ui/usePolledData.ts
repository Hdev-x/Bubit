...
export function usePolledData<T>(
  ...
  useLayoutEffect(() => { fetcherRef.current = fetcher; }, [fetcher]);
  ...
  useEffect(() => {
    ...
    return () => {
      // eslint-disable-next-line react-hooks/exhaustive-deps -- genRef는 세대 카운터(DOM ref 아님) — cleanup에서 올려 늦은 load 응답을 무효화한다
      genRef.current++;
