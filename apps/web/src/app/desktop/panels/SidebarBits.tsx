...
export function HeaderLogo({ base, logoUrl }: { base: string; logoUrl?: string }) {
  ...
  const [failure, setFailure] = useState<{ url: string | undefined; failed: boolean }>({ url, failed: false });
  if (failure.url !== url) setFailure({ url, failed: false });
  const failed = failure.url === url && failure.failed;
  return (
    <span className="sh-coin-logo">
      {url && !failed
        ? <img src={url} alt={base} onError={() => setFailure({ url, failed: true })} />
