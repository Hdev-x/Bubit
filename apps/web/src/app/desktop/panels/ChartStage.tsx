...
export type ChartDataGroup = {
  candles: Candle[];
  timeframe: Tf;
  candlesKey: string | null | undefined; // 표시 중인 캔들의 마켓 키(거래소|현선물|심볼|TF,)
  ...
  obOptions: OBOptions;
  indicatorSettings: IndicatorState['effIndicatorSettings'];
  pending: boolean;
  error: boolean;
...
// 차트 무대 — 드로잉 플로팅바·설정, RSI 설정 패널, OHLC 오버레이, MarketChart. DesktopApp에서 JSX만 옮김 .
export function ChartStage({ draw, indi, view, rsi, rank, solo, data, sel, user, chartRef, ohlc, setHoveredCandle, fmtPx, fmtVol }: {
  ...
  const { effMaSettings, effBbSetting, effPivotSetting } = indi;
  ...
  const { candles, timeframe, candlesKey, handleVisibleRangeChange, mtfCandles, mtfKey, chartTickDecimals, obOptions, indicatorSettings: effIndicatorSettings, pending, error, missing, target, onRetry } = data;
  const CHART_SYMBOL = sel.symbol;
  const CHART_PRODUCT = sel.productType;
  const chartSel = sel;
  const drawingKeys = getDrawingStorageKeys('desktop', chartSel.exchange, CHART_PRODUCT, CHART_SYMBOL);
  const interactionBlocked = pending || error || candles.length === 0;
  return (
                <div className="chart-stage">
                  {/* 드로잉 플로팅 툴바 + 설정 다이얼로그 — 도형 선택 시 */}
                  {selDrawId && !interactionBlocked && (
                    <DrawingFloatBar
                  ...
                  {selDrawId && drawSettingsOpen && !interactionBlocked && (
                    <DrawingSettings
                      getManager={() => chartRef.current?.getDrawingManager()}
                      drawingId={selDrawId}
                  ...
                  <div style={{ position: 'absolute', inset: 0 }}>
                    <MarketChart
                      ...
                      drawingStorageKey={user ? drawingKeys.key : undefined}
                      legacyDrawingStorageKey={user ? drawingKeys.legacyKey : undefined}
                    ...
                    />
                    <ChartLoadStatus pending={pending} error={error} missing={missing} target={target} onRetry={onRetry} />
                  </div>
                </div>
  );
}
