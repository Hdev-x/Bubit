import '../../../shared/ui/toss-switch.css';
import '../../../chart/indicators/indicators.css'; // 지표 섹션 규칙(.indicator-group*·.interval-*·.smc-*·.harmonic-*) — 쓰는 컴포넌트가 직접 import (: 값 re-export 경로로만 번들되던 것을 명시)
import type { AuthUser } from '../../../api/server/authApi';
import { PRESET_THEMES } from '../../../chart/settings/theme';
...
// 차트 툴바 — 타임프레임·solo 칩·신뢰선·RSI·캡쳐·그리기/지표/차트설정 드롭다운. DesktopApp에서 JSX만 옮김 .
// 상태는 묶음(draw·indi·view·rsi·rank·solo)으로 받고 첫 줄에서 풀어 쓴다. 본문은 그대로.
export function ChartToolbar({ draw, indi, view, rsi, rank, solo, user, onLoginClick, isAdmin, visibleTFs, displayedActiveTf, chartRef, handleCaptureChart, interactionBlocked = false }: {
  draw: DrawingState;
  indi: IndicatorState;
  view: ChartViewState;
  rsi: RsiGroup;
  ...
  const { setActiveTf, chartSetOpen, setChartSetOpen, chartSetRef, chartTheme, setChartTheme, isCustomTheme, isLogScale, setIsLogScale, priceLineOn, setPriceLineOn } = view;
  const { rsiOn, setRsiOn, setRsiSettingsOpen } = rsi;
  const { rankMasterOn, setRankMasterOn, rankTiers, setRankTiers } = rank;
  const { soloOn, focusTracker, setFocusTracker, setSoloActive, soloUserViewRef } = solo;
  return (
                <div className="chart-toolbar">
                  <div className="tf-bar">
                    {visibleTFs.map((t) => (
                      <button key={t} className={`tf-btn${displayedActiveTf === t ? ' active' : ''}`} onClick={() => {
                        if (soloOn) {
                  ...
                  {soloOn && (
                    ...
                    >
                      ...
                      {String(focusTracker?.symbol ?? '').replace('USDT', '')} {focusTracker?.patternName ?? '패턴'} 집중
                        ...
                        <div className="chart-dd-panel draw-panel">
                          <div className="draw-panel-scroll">
                            ...
                            <div className="draw-actions">
                              <button className="draw-act-btn" disabled={interactionBlocked || !drawHistory.canUndo} onClick={() => chartRef.current?.undo()} title="되돌리기">
                              ...
                              <button className="draw-act-btn" disabled={interactionBlocked || !drawHistory.canRedo} onClick={() => chartRef.current?.redo()} title="다시 실행">
                              ...
                              <button className="draw-act-btn danger" disabled={interactionBlocked} onClick={() => chartRef.current?.clearAll()} title="모두 삭제">
