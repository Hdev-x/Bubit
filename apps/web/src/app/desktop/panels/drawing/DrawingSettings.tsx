...
// 드로잉 설정 다이얼로그 — DrawingToolbar.tsx에서 분리 .
...
import { WIDTHS, LINE_STYLES } from './palette';
...
type DrawingSnapshot = { style: IDrawing['style']; options: IDrawing['options']; anchors: Anchor[] };
// 취소: 열 때 찍은 스냅샷으로 드로잉 객체를 되돌린다. IDrawing은 차트 라이브러리의 가변 객체(React 상태 아님)라
// 컴포넌트 밖 함수에서 mutation한다 (react-hooks/immutability).
function restoreSnapshot(drawing: IDrawing, snap: DrawingSnapshot) {
...
export function DrawingSettings({ getManager, drawingId, onClose }: {
  ...
  // 취소용 스냅샷(열 때 상태)
  const snapRef = useRef<DrawingSnapshot | null>(null);
  useEffect(() => {
  ...
  const sourceLevels = drawing?.style.levels?.length ? drawing.style.levels : levelDefaults;
  const levels: FibLevel[] = sourceLevels.map((level) => ({ ...level }));
  ...
  const cancel = () => {
    ...
    if (snap) restoreSnapshot(drawing, snap);
    onClose();
  };
