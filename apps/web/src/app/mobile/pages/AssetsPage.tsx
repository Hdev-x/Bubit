...
import { useCurrency, currencyLabel } from '../../../shared/contexts/settingsContext';
...
export default function AssetsPage({ active = true, onTabBar }: { active?: boolean; onTabBar?: (p: { x: number; y: number } | null) => void }) {
  ...
  useEffect(() => {
    ...
    fetchStatus();
    const intervalId = setInterval(refreshStatus, REFRESH_MS);
    return () => {
      mountedRef.current = false;
      clearInterval(intervalId);
    };
  }, [active, fetchStatus, refreshStatus]);
  ...
  return (
    <main className="assets-page">
      ...
      <PullToRefresh onRefresh={refreshStatus}>
