...
import TradeOrderbook from '../../../chart/orderbook/TradeOrderbook';
...
import TradeTabEditSheet from '../components/trade/TradeTabEditSheet';
...
export default function OrderPage({ symbol, active, onSelectSymbol, onProductTypeChange, onExchangeChange, onOpenChart, tradeMarketReq, onTabBar }: Props) {
  ...
  useEffect(() => {
    if (tradeMarketReq && tradeMarketReq.seq !== tradeMarketSeqRef.current) {
      ...
      setActiveTab(tradeMarketReq.market === 'futures' && !exchangeSupportsFutures ? 'spot' : tradeMarketReq.market);
    }
  }, [tradeMarketReq, exchangeSupportsFutures]);
  ...
  const [depthSymbol, setDepthSymbol] = useState(symbol);
  if (depthSymbol !== symbol) {
    setDepthSymbol(symbol);
    setDepthScale('scale2');
  }
  ...
  // ── 호가창 — useMobileOrderbook : 폴링·펀딩·단위 옵션·현재가 방향·원자 커밋 ──
  const { askRows, bidRows, maxLevelSize, buyPct, centerPrice, priceDir, depthOptions, depthLabel, fmtMid, fmtPriceOb, fundingStr, obSnap } = useMobileOrderbook({
    symbol, active, isTradeView, isFuturesMarket, isDemoExchange, tradable, depthScale, getTickDecimals, realtimePrices,
  });
  ...
  return (
    <main className="trade-page">
      ...
      {/* 8. 거래 내역 우측 사이드 탭 */}
      {/* 체결 이력 저장소는 아직 미구현이라 빈 상태를 표시하며 polling하지 않는다. */}
      <TradeHistoryDrawer
        isOpen={isHistoryOpen} onClose={() => setIsHistoryOpen(false)}
        trades={[]} displayTrades={[]} loadingTrades={false}
      ...
      <TradeSymbolSheet
        ...
        onSelect={(sym, mkt, ex) => {
          ...
          setTradeExchange(ex);
          setActiveTab(isFuturesSupported(ex) ? mkt : 'spot');
          if (ex === 'BITGET') {
            setDemoSymbol('');
            onSelectSymbol?.(sym);
          } else {
      ...
      <TradeExchangeSheet
        ...
        onSelect={(ex) => {
          setTradeExchange(ex);
          if (!isFuturesSupported(ex) && activeTab === 'futures') setActiveTab('spot');
          setDemoSymbol('');
        }}
