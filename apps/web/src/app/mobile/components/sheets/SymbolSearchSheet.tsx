...
export default function SymbolSearchSheet({ isOpen, onClose, onSelect, onSelectFixed, exchange = 'BITGET', isFutures = false }: Props) {
  ...
  const [previousView, setPreviousView] = useState({ isOpen, exchange, isFutures });
  const dragControls = useDragControls();

  if (previousView.isOpen !== isOpen || previousView.exchange !== exchange || previousView.isFutures !== isFutures) {
    setPreviousView({ isOpen, exchange, isFutures });
    if (isOpen) {
      setSearchTerm('');
      setVisibleCount(40);
  ...
  useEffect(() => {
    ...
    loader().then((data) => {
      if (!ignore) setTickers(data.filter(t => t.quoteSymbol === 'USDT'));
    }).catch(() => { /* 조회 실패 시 마지막 목록 유지 */ });

    const previousOverflow = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
    return () => { ignore = true; document.body.style.overflow = previousOverflow; };
  ...
  useEffect(() => {
    ...
    try {
      ...
      next = saved ? JSON.parse(saved) : [];
    } catch {
      next = [];
    }
    // eslint-disable-next-line react-hooks/set-state-in-effect -- 열린 시트가 외부 storage의 최신 관심종목을 다시 읽는다.
    setWatchlist(next);
  }, [isOpen, exchange, isFutures]);
                  ...
                  <div className="symbol-sort-tabs">
                    {SORT_OPTIONS.map(opt => (
                      <button
                        ...
                        onClick={() => {
                          if (opt.value === sortFilter) return;
