...
const EXCHANGE_LABELS: Record<ExchangeId, string> = { BITGET: 'Bitget', BINANCE: 'Binance', UPBIT: 'Upbit', BITHUMB: 'Bithumb' };
...
export default function TimeframeSheet({ isOpen, onClose, selectedTimeframe, onSelect, exchange = 'BITGET' }: Props) {
  const [sheetSize, setSheetSize] = useState<SheetSize>('compact');
  const [wasOpen, setWasOpen] = useState(isOpen);

  if (wasOpen !== isOpen) {
    setWasOpen(isOpen);
    if (isOpen) setSheetSize('compact');
  }
                ...
                <section key={cat} className="interval-section">
                  ...
                  <div className="interval-grid">
                    {TIMEFRAMES.filter(tf => tf.category === cat).map((tf) => {
                      const unsupported = !isTimeframeSupported(exchange, tf.value);
                      return (
                      <button
                        key={tf.value}
                        className={`interval-button ${selectedTimeframe === tf.value ? 'active' : ''}`}
                        disabled={unsupported}
                        title={unsupported ? `${EXCHANGE_LABELS[exchange]}에서 지원하지 않는 인터벌입니다.` : undefined}
