...
import { DRAWING_CATEGORIES } from './drawingCategories';
...
export default function DrawingSheet({ isOpen, onClose, activeTool, onSelectTool, onClearAll }: Props) {
  ...
  const [favorites, setFavorites] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem('tv_drawing_favorites') || '[]'); }
    catch { return []; }
  });
  const [showFavoritesOnChart, setShowFavoritesOnChart] = useState(() => {
    try { return localStorage.getItem('tv_show_favorites_on_chart') === 'true'; }
    catch { return false; }
  });
  ...
  const filteredCategories = useMemo(() => {
    // Search filtering
    if (searchQuery.trim() !== '') {
      return DRAWING_CATEGORIES.map(cat => ({
        ...cat,
