...
import { errorMessage } from '../../../shared/utils/guards';
...
export default function ApiKeyManager() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current && requestId === requestIdRef.current) setKeys(list);
    } catch (e) {
      if (mountedRef.current && requestId === requestIdRef.current) setError(errorMessage(e, '요청에 실패했습니다'));
    } finally {
  ...
  useEffect(() => {
    ...
    const requestId = ++requestIdRef.current;
    fetchApiKeys()
      .then(list => { if (!ignore && requestId === requestIdRef.current) setKeys(list); })
      .catch(e => { if (!ignore && requestId === requestIdRef.current) setError(errorMessage(e, '요청에 실패했습니다')); })
      .finally(() => { if (!ignore && requestId === requestIdRef.current) setLoading(false); });
    fetchMe().then(u => { if (!ignore) setIsAdmin(u?.role === 'ADMIN'); }).catch(() => {});
    return () => {
      ignore = true;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(errorMessage(e, '요청에 실패했습니다'));
    } finally {
      setSaving(false);
    }
  }
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: 'rgba(0,0,0,0.3)', borderRadius: 12, padding: 16, marginBottom: 20, border: '1px solid rgba(255,255,255,0.04)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
              <label className="premium-label">거래소 · Beta 계좌 연동은 Bitget만 지원</label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {EXCHANGE_OPTIONS.filter(ex => ex.id === 'BITGET').map(ex => {
