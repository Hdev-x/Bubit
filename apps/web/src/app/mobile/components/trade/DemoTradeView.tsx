...
type Quote = { key: string; center: number; decimals: number };

export default function DemoTradeView({ exchange, symbol, market }: Props) {
  const ex = EXCHANGES[exchange];
  const queryKey = `${exchange}|${market}|${symbol}`;
  const [quote, setQuote] = useState<Quote | null>(null);
  const [quoteQueryKey, setQuoteQueryKey] = useState(queryKey);

  if (quoteQueryKey !== queryKey) {
    setQuoteQueryKey(queryKey);
    setQuote(null);
  }

  const currentQuote = quote?.key === queryKey ? quote : null;
  const center = currentQuote?.center ?? null;
  const decimals = currentQuote?.decimals ?? 2;
  ...
  useEffect(() => {
    ...
    loadTickers(exchange, market).then((list) => {
      ...
      if (hit) setQuote({ key: queryKey, center: hit.last, decimals: hit.tickDecimals ?? 2 });
    }).catch(() => {});
    return () => { ignore = true; };
  }, [exchange, symbol, market, queryKey]);
