...
import { useSettings, currencyLabel } from '../../../../shared/contexts/settingsContext';
...
export default function SpotAccountSummary({
  ...
  const waitingForPrices = priceSymbols.length > 0 && !allPriced;
  const [priceGate, setPriceGate] = useState(() => ({ key: priceSymbolsKey, waiting: waitingForPrices, timedOut: false }));
  if (priceGate.key !== priceSymbolsKey || priceGate.waiting !== waitingForPrices) {
    setPriceGate({
      key: priceSymbolsKey,
      waiting: waitingForPrices,
      timedOut: waitingForPrices && priceGate.waiting ? priceGate.timedOut : false,
    });
  }
  useEffect(() => {
    if (!waitingForPrices) return;
    const id = window.setTimeout(() => setPriceGate((current) => ({ ...current, timedOut: true })), 1500);
    return () => clearTimeout(id);
  }, [priceSymbolsKey, waitingForPrices]);
  const valuationReady = !waitingForPrices || priceGate.timedOut;
