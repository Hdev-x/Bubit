...
export default function TradeSymbolSheet({ isOpen, onClose, initialMarket, exchange = 'BITGET', onSelect }: Props) {
  ...
  const [filter, setFilter] = useState<Filter>(() => isFuturesSupported(exchange) ? initialMarket : 'spot');
  ...
  const [previousView, setPreviousView] = useState({ isOpen, initialMarket, exchange });
  // 실시간 WS 시세 — 거래소·마켓·심볼로 구분해 보이는 행에 덮어씀.
  ...
  if (previousView.isOpen !== isOpen || previousView.initialMarket !== initialMarket || previousView.exchange !== exchange) {
    setPreviousView({ isOpen, initialMarket, exchange });
    if (isOpen) {
      setSearchTerm('');
      setVisibleCount(40);
      setSheetSize('compact');
      setFilter(isFuturesSupported(exchange) ? initialMarket : 'spot');
  ...
  useEffect(() => {
    if (!isOpen) return;
    let next: string[];
    try { next = JSON.parse(localStorage.getItem(FAV_KEY) || '[]'); } catch { next = []; }
    // eslint-disable-next-line react-hooks/set-state-in-effect -- 열린 시트가 외부 storage의 최신 즐겨찾기를 다시 읽는다.
    setFavorites(next);
  }, [isOpen, initialMarket, exchange]);
  ...
  useEffect(() => {
    ...
    const load = () => {
      if (sheetExchange === 'BITGET') {
        fetchCoinTickers().then(d => { if (!ignore) setSpotTickers(d.filter(t => t.quoteSymbol === 'USDT' && !isLikelyStock(t))); }).catch(() => { /* 일시 실패 시 마지막 목록 유지 */ });
        fetchCoinFuturesTickers().then(d => { if (!ignore) setFuturesTickers(d.filter(t => t.quoteSymbol === 'USDT' && !isLikelyStock(t))); }).catch(() => { /* 일시 실패 시 마지막 목록 유지 */ });
      } else if (sheetExchange === 'BINANCE') {
        fetchBinanceSpotTickers().then(d => { if (!ignore) setSpotTickers(d.filter(t => t.quoteSymbol === 'USDT')); }).catch(() => { /* 일시 실패 시 마지막 목록 유지 */ });
        fetchBinanceFuturesTickers().then(d => { if (!ignore) setFuturesTickers(d.filter(t => t.quoteSymbol === 'USDT')); }).catch(() => { /* 일시 실패 시 마지막 목록 유지 */ });
      } else if (sheetExchange === 'UPBIT') {
        fetchUpbitSpotTickers().then(d => { if (!ignore) { setSpotTickers(d); setFuturesTickers([]); } }).catch(() => { /* 일시 실패 시 마지막 목록 유지 */ });
      } else if (sheetExchange === 'BITHUMB') {
        fetchBithumbSpotTickers().then(d => { if (!ignore) { setSpotTickers(d); setFuturesTickers([]); } }).catch(() => { /* 일시 실패 시 마지막 목록 유지 */ });
      }
    };
    load();
  ...
  function chooseExchange(nextExchange: ExchangeId) {
    setSheetExchange(nextExchange);
    if (!isFuturesSupported(nextExchange) && filter === 'futures') {
      setFilter('spot');
      setVisibleCount(40);
    }
  }
  ...
  useEffect(() => {
    ...
    const sub = subscribeBitgetSpotTickers(visibleSpotKey.split(','), (tk) => {
      if (!ignore) setLiveMap(prev => ({ ...prev, [`BITGET|spot|${tk.symbol}`]: tk }));
    });
    return () => { ignore = true; sub.close(); };
  ...
  useEffect(() => {
    ...
    const sub = subscribeBitgetFuturesTickers(visibleFuturesKey.split(','), (tk) => {
      if (!ignore) setLiveMap(prev => ({ ...prev, [`BITGET|futures|${tk.symbol}`]: tk }));
    });
    return () => { ignore = true; sub.close(); };
        ...
        <>
          ...
          >
            ...
            <div className="trade-symbol-market-tabs">
              <button className={filter === 'favorites' ? 'active' : ''} onClick={() => chooseFilter('favorites')}>Favorites</button>
              <button className={filter === 'spot' ? 'active' : ''} onClick={() => chooseFilter('spot')}>Spot</button>
              {isFuturesSupported(sheetExchange) && (
                <button className={filter === 'futures' ? 'active' : ''} onClick={() => chooseFilter('futures')}>Futures</button>
              )}
            </div>
              ...
              <section className="interval-section">
                ...
                <div className="symbol-row-list">
                  {visible.map(({ t, market }) => {
                    ...
                    const live = liveMap[`${sheetExchange}|${market}|${t.symbol}`];
