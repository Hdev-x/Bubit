...
다섯 가지를 검사한다.
 1. 분할 전 CSS 규칙 순서 snapshot 대비 — 같은 단순선택자·같은 속성을 다투는 규칙 쌍에서 기준과 순서가
 ...
 2. 진입점(main.tsx) import 순서 — 셸 CSS import가 컴포넌트를 끌어오는 첫 import보다 앞에 있는지(`import type`은 제외).
    셸 CSS가 컴포넌트 CSS 뒤에 번들돼 테마·flex 규칙을 덮는 회귀를 막는다.
 3. 빌드 산출물(dist/, dist-desktop/)이 있으면 번들 CSS에서 셸 규칙이 모든 컴포넌트 CSS 파일보다 앞에 있는지.
 5. 두 앱 셸(mobile.css·desktop.css)에 같은 클래스 선택자가 없는지 — 양 앱 공용 규칙은 shared/ui·chart/ 한 곳에,
    앱별 값 차이는 CSS 변수로. `:root`·`*`·`body` 같은 element·전역 선택자는 앱별 셸 루트라 비교하지 않는다(이름 예외 목록은 없다).
    선택자만 비교하고 media 조건은 무시한다(표기 차이·기본 vs media 내부 중복 방지).
 4. 번들에서 회귀 대상 선택자의 최종 선언이 기대값인지(.coin-chart-page 배경·색·하단 여백은 전용 규칙이며 값이 CoinChartPage.css와 같음, .show-current-label은 flex),
    .chart-tool-strip의 media 조건(<=410px·>=860px, 방향·값 정확 비교)이 남아 있는지.
...
ORIGIN_SNAPSHOT = os.path.join(os.path.dirname(__file__), 'fixtures', 'css-cascade-origin.json')
APPS = [
    dict(app='mobile', shell='src/app/mobile/styles/mobile.css', comp_globs=['src/app/mobile/**/*.css', 'src/chart/**/*.css', 'src/shared/**/*.css'],
         entry='src/app/mobile/main.tsx', dist='dist',
         # (선택자, 속성, 기대값). 'SOURCE:<파일>'은 그 파일의 단독 선택자 규칙에 적힌 값이 번들 최종값이어야 한다는 뜻
         # 단독 선택자인지만 보면 값이 바뀐 회귀를 통과시키므로 값도 비교한다.
         ...
         # (선택자, media 조건) — 조건은 '<=410px' 형태로 정규화해 정확히 비교한다.
         media=[('.chart-tool-strip', '<=410px'), ('.chart-tool-strip', '>=860px')]),
    dict(app='desktop', shell='src/app/desktop/styles/desktop.css', comp_globs=['src/app/desktop/**/*.css', 'src/chart/**/*.css', 'src/shared/**/*.css'],
         entry='src/app/desktop/main.tsx', dist='dist-desktop',
         final=[], media=[]),
]
...
def simple(sel):
    """콤마 목록을 단순 선택자로 나눈다 — :is(.a, .b)·:not(...) 안의 콤마는 나누지 않는다."""
    out, cur, depth = [], '', 0
    for ch in sel:
        if ch in '([':
            depth += 1
...
def class_tokens(sel):
    """선택자의 실제 class token — attribute 값·문자열 안의 점(a[href="v1.0"])은 제외한다."""
    s = re.sub(r'\[[^\]]*\]', '', sel)
    s = re.sub(r'"[^"]*"|\'[^\']*\'', '', s)
...
def rule_fingerprint(selector, body):
    return hashlib.sha256((selector + '\0' + body).encode()).hexdigest()


def load_origin_positions(app):
    try:
        with open(ORIGIN_SNAPSHOT, encoding='utf-8') as f:
            snapshot = json.load(f)
        if snapshot.get('schema') != 1 or snapshot.get('fingerprint') != 'sha256(selector+NUL+body)':
            raise ValueError('지원하지 않는 snapshot 형식')
        return snapshot['apps'][app]
    except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as e:
        raise RuntimeError(f'CSS 순서 snapshot을 읽을 수 없다: {e}') from e


def check_origin_order(cfg, shell_rules, comp_rules):
    opos = load_origin_positions(cfg['app'])

    def origin_position(rule):
        return opos.get(rule_fingerprint(*rule))
    ...
    for f, s, b in comp_rules:
        for (sh_s, sh_b) in shell_rules:
            if not competing((sh_s, sh_b), (s, b)):
                continue
            oi_shell, oi_comp = origin_position((sh_s, sh_b)), origin_position((s, b))
        ...
        for i in range(len(rs)):
            for j in range(i + 1, len(rs)):
                if not competing(rs[i], rs[j]):
                    continue
                oi, oj = origin_position(rs[i]), origin_position(rs[j])
...
def check_shell_duplicates():
    """[5] 두 셸의 공통 클래스 선택자 0 — 최상위 콤마로 나눈 단순 선택자 중 class token이 있는 것만 비교(element·전역 선택자 제외).
    media 조건은 비교 키에 넣지 않는다(표기 차이·기본 규칙 vs media 내부 중복을 놓치지 않도록). 보고에는 media를 함께 적는다."""
    maps = []
    for cfg in APPS:
        rs = rules(open(cfg['shell'], encoding='utf-8').read())
        mp = {}
        for m, s, b in rs:
            for x in simple(s):
                if class_tokens(x):
                    mp.setdefault(x, set()).add(m)
        maps.append(mp)
    common = sorted(set(maps[0]) & set(maps[1]))
    print(f'shells:\n  [5] 셸 공통 클래스 선택자: {len(common)}개')
    def media_note(x):
        ms = sorted(maps[0][x] | maps[1][x])
        return ' (' + ' / '.join(m or '기본' for m in ms) + ')' if ms != [''] else ''
    return [f'DUP 셸 공통 선택자 "{x}"' + media_note(x) for x in common]


def main():
    total = 0
    problems = check_shell_duplicates()
    ...
    for cfg in APPS:
        ...
        comp_files = [f for f in sum((glob.glob(g, recursive=True) for g in cfg['comp_globs']), [])
